  ...
  for k in range(n):
    for j in range(n):
      for i in range(n):
        ...
        neighbors = np.zeros(27, dtype=jnp.int32)
        ct = 0
        for k_ in (k-1, k, k+1):
          for j_ in (j-1, j, j+1):
            for i_ in (i-1, i, i+1):
              neighbors[ct] = compute_id(i_%n, j_%n, k_%n, n)
              ct += 1
...
distance_vec = jax.jit(jax.vmap(distance, in_axes=(None,0, None), out_axes=0))
...
# @lru_cache(maxsize=None)    ####### TODO Remember this info 
def find_points_in_cell(cell_id, points_to_cells):
  # return jnp.argwhere(points_to_cells==cell_id, size=points_to_cells.shape[0]//jnp.max(points_to_cells), fill_value=-1)[:,0]
  # point_ids = jnp.argwhere(points_to_cells==cell_id, size=5, fill_value=-1)[:,0]    ## TODO Use all cells
